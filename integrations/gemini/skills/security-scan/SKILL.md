...
2. Use Tomofound MCP tools first:
   - `discover_targets` with `target: "gemini"` to inventory installed Gemini targets (default when no target specified from Gemini CLI). For other platforms or local paths, pass the appropriate `target` or `path` parameter.
...
3. Inventory files by role:
   ...
   - Instruction files: `GEMINI.md`, `SKILL.md`, `AGENTS.md`, skill, agent, command, and prompt Markdown or TOML.
...
6. Handle `atr_scan_path` budget limits:
   - **Do NOT pass the whole `~/.gemini/` tree in one call.** Walk the `discover_targets` output and call `atr_scan_path` once per distinct plugin / skill / extension root.
   - If `budget_exceeded: true` comes back, re-invoke on narrower subpaths until every subtree has been fully scanned. Merge findings across calls.
   - If partial coverage is unavoidable, mark the affected item with a partial-coverage warning in the report.
7. Report findings with severity, category, file, line when known, evidence snippet, impact, and remediation.

## Output

Use the following report shape when producing a full scan report:
